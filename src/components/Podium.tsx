...
function Step({ share, tier }: { share: ShareResult; tier: Tier }) {
    ...
    <div className="flex flex-1 flex-col items-center justify-end">
      <div className="mb-2 flex flex-col items-center text-center">
        ...
        <span className="text-[11px] font-semibold uppercase tracking-wide text-neutral-400">
          {formatPct(share.pct)} · {share.label}
        </span>
            ...
            <>
              ...
              <div
                className={`text-xs font-medium ${
                  share.recipientType === 'charity' ? 'text-pitch-600' : 'text-neutral-500'
                }`}
              >
  ...
  return (
    <div className="card p-5 sm:p-6">
      ...
      <div className="flex items-end gap-3 sm:gap-4">
        <Step share={runnerUp} tier="silver" />
        <Step share={champion} tier="gold" />
        <Step share={third} tier="bronze" />
      </div>
    </div>
  )
}
